    ...
    public User[] minDistanceUsers( int nSamples, User A, int[][] pathGridCells, boolean shopper ){
        ...
        for ( int[] cell : pathGridCells ){
            ...
            for ( User u : this.buyerDatabase.usersOfCell( cellX, cellY ) ){
                if ( u != null ){
                    if ( !u.isReserved() ){
                        if ( A.sameShop() || u.sameShop() ){
                            Coordinate Ashop = A.getShopCoordinate();
                            Coordinate Ushop = u.getShopCoordinate();

                            //Same shop approximation
                            if ( Ashop.distance(Ushop) > 0.001 ){
                                continue;
                            }
                        }
                        ...
                        double d;
                        double pathID;
                        if (shopper){ returnType = GraphSystem.minGraphDistance( A, u,1, this.gsLongtitude, this.gsLatitude, A.sameShop() || u.sameShop() ); }
                        else{ returnType = GraphSystem.minGraphDistance( u, A, 1,  this.gsLongtitude, this.gsLatitude, A.sameShop() || u.sameShop() ); }
